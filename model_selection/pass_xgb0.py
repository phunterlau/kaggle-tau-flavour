...
def cv_model_list():
	model_list = [(RF(n_estimators = 300, n_jobs = -1, criterion = 'entropy'),'rf_1')]
	for num_round in [200]:
		for max_depth in [5]:
			for eta in [0.1]:
				for min_child_weight in [1,2,5]:
					for colsample_bytree in [0.25,0.5,0.7,0.8]:
						for subsample in [0.2,0.3,0.5,0.8]:
...
def cv_model(model_list):
	...
	for j, (clf, clf_name) in enumerate(model_list):
		...
		if ks <= 0.09 and cvm <= 0.002 and auc_score > 0.975:
			# save the cv
